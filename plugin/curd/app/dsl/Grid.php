 ...
 *
 * 输出：->columns() / ->search() / ->formFields() / ->options() 供 BaseCurdController::config() 使用
 */
class Grid extends BaseDsl
{
     ...
     *
     *   $grid->addAction(new PaymentAuditAction());       // plugin/curd/actions/PaymentAuditAction.php
     *   $grid->addAction(new PaymentRejectAction());
     *
     * 类写法（plugin/curd/actions/ 下，继承 plugin\curd\app\dsl\Action，在 init() 里配置）：
     ...
     * @param string $modelName 模型短名（如 Payment），用于拼 /api/curd/model/{model}/action/{name}
     */
