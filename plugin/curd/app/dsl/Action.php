...
class Action extends BaseDsl
{
    ...
    /** @var string 控制器处理方法名（自动拼 /api/curd/model/{model}/action/{name}） */
    protected $handler = '';
    ...
    protected $roles = [];
    /** @var string|null 权限资源对象，如 curd.Payments */
     ...
     *       ...
     *   }
     * 未覆盖时由 CurdActionsTrait 回退调用控制器上的 action{Name}(Request $request)。
     */
    public function handle(Request $request)
    {
    ...
    /**
     * 控制器处理方法名：自动指向 /api/curd/model/{model}/action/{name}
     ...
     * 例如 ->permission('curd.Payments', 'audit')。
     */
    ...
    public function resolveApi($modelName)
    {
        if ($this->api === '') {
            ...
            if ($handler !== '') {
                $this->api = '/api/curd/model/' . $modelName . '/action/' . $handler;
            }
        }
    }
