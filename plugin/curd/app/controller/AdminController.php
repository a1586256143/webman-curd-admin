 ...
 * 后台系统管理：用户 / 角色 / 权限
 *
 * 数据全部来自独立认证库（连接名 config('plugin.curd.curd.admin_connection')，默认 mysql），
 * 与业务库解耦。
 *
 * 注意：
 ...
 *
 * 路由：plugin/curd/config/route.php 注册 /api/admin/* 与 /api/config/site。
...
class AdminController
{
     ...
     *    → 容器 slug(curd.{Model}) 写入后，菜单 can(curd.{Model}) 可见。
