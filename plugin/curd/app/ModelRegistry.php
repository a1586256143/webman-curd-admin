 ...
 * 只有继承 support\Model 的类才允许 CURD，前端无法通过换表名探查其他表。
 *
 ...
 *
 * 注册方式（bootstrap.php）：
 *   \plugin\curd\app\ModelRegistry::registerModel(\app\model\APackage::class);
 *   \plugin\curd\app\ModelRegistry::registerModel(\app\model\AdminUser::class);
 *   或批量自动扫描：
 *   \plugin\curd\app\ModelRegistry::scanModels(); // 扫描 app/model 目录（不含 BaseModel）
 *
 * 解析：
...
class ModelRegistry
{
    ...
    /**
     * 显式允许动态解析的表名（启动时从 curd_configs 已登记的表加载）
     */
    ...
    /**
     * 设置允许动态解析的表名白名单（curd_configs 已登记的表）
     */
    ...
    {
        $dir = self::$modelDir ?: (string)config('plugin.curd.curd.model_dir', base_path() . '/app/model');
        if (!is_dir($dir)) {
            return 0;
        }
    ...
    /**
     * 扫描控制器目录，自动把继承 BaseCurdController 的控制器注册为模型关联
     * （供 /api/curd/config?model=xxx 拿页面 grid 配置；无专属 controller 的模型可跳过）
     */
    public static function scanControllers(): int
    {
        // 控制器目录 => 命名空间（兼容 app/controller/api 与 app/controller/admin/api，
        // 默认值见 config/plugin/curd/curd.php controller_dirs，新项目可按需调整）
        // make:curd 默认生成到 app/controller/api，但实际业务控制器可能放在 admin/api 下，
        // 必须两个目录都扫描，否则专属控制器无法与模型关联，通配路由会回退到 DynamicCurdController，
        // 导致 grid()/display 处理器/业务钩子全部失效。
        $dirs = config('plugin.curd.curd.controller_dirs', []);
        $count = 0;
        foreach ($dirs as $dir => $namespace) {
            ...
            foreach (glob($dir . '/*.php') ?: [] as $file) {
                ...
                if (class_exists($class)
                    && is_subclass_of($class, controller\base\BaseCurdController::class)) {
    ...
    public static function resolve(string $name): ?string
    {
        ...
        // 仅当表在白名单内（curd_configs 已登记 / 显式开启）才允许，防止猜模型名访问任意表
    ...
    protected static function generateModel(string $name, string $table): void
    {
        ...
        // 动态生成的模型继承的基类（config('plugin.curd.curd.base_model_class')，
        $baseClass = (string)config('plugin.curd.curd.base_model_class', '\app\model\BaseModel');
    ...
    {
        $model = self::instance($name);
        return $model ? ($model->getConnectionName() ?: CurdDb::business()) : null;
    }
    ...
    public static function resolveTable(string $name): ?array
    {
        ...
        return [$model->getConnectionName() ?: CurdDb::business(), $model->getTable()];
    }
    ...
    public static function registerFromController(string $controllerClass): void
    {
        ...
        if (!is_subclass_of($controllerClass, controller\base\BaseCurdController::class)) {
            return;
        }
