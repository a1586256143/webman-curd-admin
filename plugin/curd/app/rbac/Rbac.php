 ...
 *
 * 用法:
 *   Rbac::enforce($userId, 'curd.APackage', 'list');   // bool
 *   Rbac::check($userId, 'curd.APackage', 'list');     // 无权限抛异常
 *
 * 规则说明:
 *   p = sub, obj, act        策略: 角色/用户 对 资源 的操作
 *   g = _, _                 角色继承(用户属于角色 / 角色继承角色)
 *   模型文件: plugin/curd/config/casbin.conf（随插件分发）
...
class Rbac
{
    ...
    public static function enforcer(): Enforcer
    {
        if (self::$enforcer === null) {
            ...
            $modelPath = (string)config('plugin.curd.curd.casbin_model_path', '');
