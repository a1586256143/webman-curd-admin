...
use plugin\crud\app\controller\base\DynamicCrudController;
use plugin\crud\app\CrudConfigRegistry;
 ...
 * 2. 通配 CRUD（/api/crud/model/{model}/*）—— 不再复制实现，统一收敛到 BaseCrudController：
 *    - 模型有专属业务控制器（继承 BaseCrudController，如 PackageController）→ 直接复用其实例，
 *      业务钩子（createdAfter 等）与 grid 配置天然生效
 *    - 没有专属控制器 → 用 DynamicCrudController 按模型动态复用基类能力
 *    这样 CRUD 逻辑只有 BaseCrudController 一份，删除了本类里重复的 list/export 等实现。
 */
class CrudController
{
    public static function getCrudConfig(){
        // 明确走认证库连接（crud_configs 位于认证库）
        return \plugin\crud\app\model\CrudConfigs::on(CrudDb::admin())->newQuery();
    }
    ...
    protected function resolveModel(string $model): ?string
    {
        ...
        // fallback：crud_configs 表里的 table_name 反查已注册模型
        // （兼容历史配置：保存时落库的是 table_name）
        try {
            $row = CrudConfigs::firstByTableName($model);
            if ($row) {
    ...
    /**
     * 获取所有已注册的 CRUD 配置列表（主库）
     */
    public function configList()
    {
        $list = CrudConfigRegistry::all();
    ...
    public function config(Request $request)
    {
        ...
        // 优先按 table_name 查找（主库 crud_configs）
        $configRow = CrudConfigs::firstByTableName($identifier);

        // 再按 route_path 查找（严格大小写敏感，注册路径与访问路径必须一致）
        if (!$configRow) {
            try {
                $configRow = CrudConfigs::firstByRoutePath($identifier);
            } catch (\Throwable $e) {
                // route_path 字段不存在，忽略
            }
        }
        ...
        // 如果数据库没有，尝试从注册表获取
        $config = CrudConfigRegistry::get($identifier);
        if ($config) {
            return json(['code' => 200, 'msg' => 'success', 'data' => $config->toFrontendConfig()]);
        }

        // 再尝试从路由→控制器注册表获取（继承 BaseCrudController 的控制器动态配置）
        if (is_string($identifier)) {
            $controller = \plugin\crud\app\RouteControllerRegistry::instance($identifier);
            if ($controller) {
                ...
                // 只暴露模型名，不暴露表名
                // 优先用 config() 已输出的 _modelName（BaseCrudController 已改造）
    ...
    /**
     * 表名 → 模型名（兜底，从表名反推驼峰）
     * 与 MakeCrudCommand::tableToModelName 规则一致：
     *   a_tails     → ATail
     *   a_packages  → APackage
    ...
    {
        $configs = CrudConfigRegistry::allConfigs();
    ...
    public function saveConfig(Request $request)
    {
        ...
        $exists     = self::getCrudConfig()->where('table_name', $table)->exists();
        ...
        try {
            if ($exists) {
                ...
                self::getCrudConfig()->where('table_name', $table)->update($updateData);
            } else {
                $insertData = [
                    'table_name' => $table,
                    'title'      => $title,
                ...
                self::getCrudConfig()->insert($insertData);
            }
        } catch (\Throwable $e) {
            if (str_contains($e->getMessage(), 'route_path')) {
                if ($exists) {
                    self::getCrudConfig()->where('table_name', $table)->update([
                        'title'      => $title,
                        'page_name'  => $pageName,
                        'config'     => $configJson,
                        'updated_at' => $now,
                ...
                return json(['code' => 200, 'msg' => '配置保存成功！建议执行 SQL 添加 route_path 字段：ALTER TABLE crud_configs ADD COLUMN route_path VARCHAR(255) NULL UNIQUE AFTER page_name;']);
            }
            throw $e;
        }
    ...
    public function generate(Request $request)
    {
        ...
        foreach ($columns as $col) {
            ...
            if ($isSearchable && !in_array($field, ['id', 'created_at', 'updated_at'])) {
                ...
                // datetime 在搜索栏转为 daterange（GenericCrud 搜索栏支持 date/daterange）
    ...
    // ============================================================
    // 通配 CRUD：统一收敛到 BaseCrudController（薄转发）
    // ============================================================

    /**
     * 解析模型名 → BaseCrudController 实例
     * 优先级：专属业务控制器（继承 BaseCrudController）→ DynamicCrudController 兜底
     */
    protected function resolveCrud(string $model): ?BaseCrudController
    {
        // 1. 有专属业务控制器（如 PackageController，继承 BaseCrudController 且带 grid/钩子）→ 复用其实例
        $controller = ModelRegistry::controller($model);
        if ($controller !== null && class_exists($controller)
            && is_subclass_of($controller, BaseCrudController::class)) {
        ...
        return DynamicCrudController::forModel($modelClass);
    }
    ...
    {
        $crud = $this->resolveCrud($model);
        if ($crud === null) {
            return json(['code' => 404, 'msg' => "模型 {$model} 未注册"]);
        }
        return $crud->index($request);
    }
    ...
    public function delete($model, Request $request)
    {
        ...
        return $crud->delete($request);
    }

    /**
     * 自定义操作分发（Action DSL ->handler('xxx') 自动指向这里）
     * URL: POST /api/crud/model/{model}/action/{name}
    ...
    public function action($model, $name, Request $request)
    {
        ...
        return $crud->action($name, $request);
    }
    ...
    public function batchDelete($model, Request $request)
    {
        ...
        return $crud->batchDelete($request);
    }
