 ...
 *
 * 用法：
 *  - 前端以 VITE_BASE_PATH=<page_base> 构建，产物整体放入 plugin/crud/public/
 *    （page_base 默认 /app/crud，config/plugin/crud/crud.php 可调，前后端需一致）
 *  - 本控制器把 <page_base>/* 下真实存在的文件按静态文件返回（带 mime/cache 头）；
 *    其余路径（vue-router history 路由，如 /app/crud/dashboard）统一回退 index.html
...
class PageController
{
    ...
    {
        $publicDir = (string)config('plugin.crud.crud.public_dir', dirname(__DIR__, 2) . '/public');
        $indexFile = rtrim($publicDir, '/') . '/index.html';

        if (!is_dir($publicDir) || !is_file($indexFile)) {
            // 未内置前端（未构建或独立部署）：提示而非 404 迷宫
            return response('CRUD 插件内置前端未安装：请先构建前端产物到 plugin/crud/public/（见插件 README）', 404);
        }
