...
> 当前插件形态（webman 应用插件）：
> - 命名空间为 `plugin\crud\app\...`，源码位于 `plugin/crud/app/`
> - 路由为自定义 `/api/*`（见 `plugin/crud/config/route.php`），并非官方应用插件默认的 `/app/crud/*`
> - 初始化通过 `plugin/crud/config/autoload.php` 加载 `app/bootstrap.php`
> - 下方旧示例为历史文档，命名空间/路径以实际代码为准

## 概述

通过 PHP 类定义 CRUD 配置，前端通过接口动态获取配置，实现配置与代码分离。

## 文件结构

```
webman-admin-api/plugin/crud/
├── CrudConfig.php          # 配置基类
├── OrderCrudConfig.php     # 订单管理配置示例
├── UserCrudConfig.php      # 用户管理配置示例
├── CrudConfigRegistry.php  # 配置注册中心
...
在 `webman-admin-api/plugin/crud/` 目录下创建新的配置类：

```php
<?php
namespace plugin\crud;

use plugin\crud\CrudConfig;

/**
 * 商品管理 CRUD 配置
 */
class ProductCrudConfig extends CrudConfig
{
    /**
     * 配置唯一标识
     */
...
在 `webman-admin-api/plugin/crud/bootstrap.php` 中添加注册：

```php
<?php
use plugin\crud\CrudConfigRegistry;

// 注册订单管理
CrudConfigRegistry::register('order', \plugin\crud\OrderCrudConfig::class);

// 注册用户管理
CrudConfigRegistry::register('user', \plugin\crud\UserCrudConfig::class);

// 注册商品管理（新增）
CrudConfigRegistry::register('product', \plugin\crud\ProductCrudConfig::class);
```
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/crud/config/list` | GET | 获取所有已注册配置 |
| `/api/crud/config?key=xxx` | GET | 获取单个配置 |
| `/api/crud/config/all` | GET | 获取所有配置详情 |
| `/api/crud/tables` | GET | 获取数据库所有表 |
| `/api/crud/generate?table=xxx` | GET | 从数据库表生成配置 |
...
1. **复制后端文件**
   - 复制整个 `webman-admin-api/plugin/crud/` 目录

2. **检查配置**
   - 确保 `config/autoload.php` 包含 `bootstrap.php`：
   ```php
   base_path() . '/plugin/crud/bootstrap.php',
   ```
