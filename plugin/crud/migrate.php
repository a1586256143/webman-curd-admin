...
<?php
/**
 * CRUD 插件 · 增量 migration 运行器
 *
 * 约定：
 *  - 迁移文件放在 plugin/crud/migrations/ 目录，命名 V{版本}__{描述}.sql
 *    例：V1__create_goods.sql  V2__add_goods_stock.sql  V10__index_orders.sql
 *  - 版本号为纯数字，按数字升序执行；同一版本内的多条语句用 --SPLIT-- 分隔
 *  - 已执行的迁移记录进 {连接}.{前缀}_crud_migrations 表，幂等：只跑未执行的
 *  - 运行库连接走 config('plugin.crud.crud.business_connection')（默认 mysql_business），
 *    可用 --connection=<name> 覆盖；跟踪表建立在该连接上
 *
 * 用法（在 webman 宿主根目录执行）：
 *   php plugin/crud/migrate.php                 # 跑全部未执行迁移
 *   php plugin/crud/migrate.php --connection=mysql_business
 *   php plugin/crud/migrate.php --dry-run      # 只列出将要执行的文件，不落库
 *
...
$connection = $connection ?? (string)config('plugin.crud.crud.business_connection', 'mysql_business');
...
// 确保跟踪表存在
$db->statement(
    "CREATE TABLE IF NOT EXISTS `_crud_migrations` (
...
$applied = $db->table('_crud_migrations')->pluck('version')->map(fn($v) => (int)$v)->all();
...
$batch = ((int)$db->table('_crud_migrations')->max('batch')) + 1;
$ok = true;
foreach ($toRun as [$version, $f]) {
    $content = file_get_contents($f);
    // 复用 Install 的分隔解析（isContent=true）
    try {
        $n = \plugin\crud\api\Install::runSqlFile($content, $db, true);
        $db->table('_crud_migrations')->insert([
            'version' => $version,
            'file' => basename($f),
            'batch' => $batch,
