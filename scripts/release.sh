#!/usr/bin/env bash
# 发布脚本：把宿主开发版 plugin/crud 回灌到本 composer 包后提交
...
if [ ! -d "$HOST/plugin/crud" ]; then
  echo "未找到宿主插件目录: $HOST/plugin/crud" >&2
  exit 1
fi
...
rsync -a --delete \
  ...
  --exclude '.DS_Store' \
  "$HOST/plugin/crud/" "$PKG/plugin/crud/"

echo "已同步 $HOST/plugin/crud → $PKG/plugin/crud"
