#!/usr/bin/env bash
#
# 构建插件内置前端并回灌到 plugin/crud/public/
#
# 用法：
#   ./scripts/build-frontend.sh [前端源码目录]
#
# 前端源码目录默认取环境变量 CRUD_FRONTEND_DIR，未设置则报出提示退出。
# 产物以 VITE_BASE_PATH=/app/crud/ 构建（与 plugin/crud/config/crud.php 的
...
PUBLIC_DIR="$PLUGIN_DIR/plugin/crud/public"
FRONTEND_DIR="${1:-${CRUD_FRONTEND_DIR:-}}"

if [ -z "$FRONTEND_DIR" ]; then
  echo "用法: $0 <前端源码目录>   （或设置环境变量 CRUD_FRONTEND_DIR）"
  exit 1
fi
...
PAGE_BASE="${CRUD_PAGE_BASE:-/app/crud/}"
